...
namespace Thomas {
	...
	void Camera::Camera2D_compute_world_to_ndc_xform() {
		...
		camwin_to_ndc_xform = { 1 / ar,0,0,0,-1,0,0,0,1 };
	...
	void Camera::Camera2D_Update() {
		if (camtype_flag == GL_TRUE) {
			...
			std::cout << "Camera Tog:     " << cam_tog << std::endl;
		...
		if (left_turn_flag == GL_TRUE) {
			...
			std::cout << "Turning Left:     " << rotation << std::endl;
		}
		...
		if (right_turn_flag == GL_TRUE) {
			...
			up = { -(sin(rotation * (M_PI / 180))), cos(rotation * (M_PI / 180)) };
			right = { cos(rotation * (M_PI / 180)), sin(rotation * (M_PI / 180)) };
			std::cout << "Turning Right:     " << rotation << std::endl;
		}
	...
	void Camera::Camera2D_Update(Transform& selected_obj) {
		...
		if (left_turn_flag == GL_TRUE) {
			...
			std::cout << "Turning Left:     " << selected_obj.rotation << std::endl;
		}
		...
		if (right_turn_flag == GL_TRUE) {
			...
			up = { -(sin(selected_obj.rotation * (M_PI / 180))), cos(selected_obj.rotation * (M_PI / 180)) };
			right = { cos(selected_obj.rotation * (M_PI / 180)), sin(selected_obj.rotation * (M_PI / 180)) };
			std::cout << "Turning Right:     " << selected_obj.rotation << std::endl;
		}
