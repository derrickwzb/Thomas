...
void Graphics::update(std::vector<Thomas::Entity> allentity){
	...
	for (auto const& entity : allentity) {
		...
		if (Thomas::factory.HasComponent<Transform>(entity)) {
			...
			glm::mat3 scale_mat = { trans_data.scaling.x, 0, 0, 0, trans_data.scaling.y, 0, 0, 0, 1 };
			glm::mat3 rot_mat = { cos(trans_data.rotation * (M_PI / 180)), sin(trans_data.rotation * (M_PI / 180)), 0, -sin(trans_data.rotation * (M_PI / 180)), cos(trans_data.rotation * (M_PI / 180)), 0, 0, 0, 1 };
			glm::mat3 trans_mat = { 1, 0, 0, 0, 1, 0, trans_data.translation.x, trans_data.translation.y, 1 };
			trans_data.mdl_to_ndc_xform = cam_data.world_to_ndc_xform * trans_mat * rot_mat * scale_mat;
