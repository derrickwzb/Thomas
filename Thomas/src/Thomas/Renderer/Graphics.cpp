...
namespace Thomas {
	...
	void Graphics::update(std::vector<Thomas::Entity> allentity) {
		...
		for (auto const& entity : allentity) {
			...
			// CAMERA
			if (Thomas::factory.HasComponent<Camera>(entity)) {
				auto cam_data = Thomas::factory.GetComponent<Camera>(entity);
				auto trans_data = Thomas::factory.GetComponent<Transform>(entity);

				// If camera is not tag to any object
				if(cam_data.cam_mode == 1)
					cam_data.Camera2D_Update();

				// If camera is tag to an object, put the tag object as the parameter
				if (cam_data.cam_mode == 2)
					cam_data.Camera2D_Update(trans_data);
			...
			if (Thomas::factory.HasComponent<Transform>(entity)) {
				...
				Thomas::factory.UpdateComponent<Camera>(entity, cam_data);
				Thomas::factory.UpdateComponent<Transform>(entity, trans_data);
			}
